...
image_labels = {
    "home": [
        ...
        "Search Strategy — Image",
        "Future-proof Strategy — Image",
        "Search Trends — Image",
        "Closing Call to Action — Image",
    ],
    ...
    "search-engine-marketing": [
        ...
        "Visibility — Image",
        "PPC Services — Image",
        "Google Ads — Certification Image",
        "Microsoft Advertising — Certification Image",
        "FAQs — Image",
    ],
    "blog-detail": [
        "Article — Featured Image",
        "Local Rankings — Diagram",
        "Related Post 1 — Image",
        "Related Post 2 — Image",
        "Related Post 3 — Image",
    ],
    "case-study-detail": [
        "Case Study — Featured Image",
        "Challenge — Image",
        "Results — Search Performance Chart",
    ],
}
names = {
    "home": "Home",
...
shared_labels = {
    "shared_78f8d1b3d0a8": "Footer — Navigation Heading",
    "shared_63ce494a885e": "Footer — Services Heading",
    "shared_4937e4e54c2c": "Footer — Contact Heading",
    "shared_7b3ca0dbb8c9": "Footer — Call Button Label",
    "shared_a8e4f9f45c81": "Footer — Copyright and Legal Text",
}
groups = []
shared = {}
...
def link_field(t, slug):
    text = t["text"].strip()
    return bool(
        re.match(r"^(Get Started|Schedule|Take the first)", text, re.I)
        or text in ["Case Studies", "Careers", "Read More", "Learn More"]
        or (text.startswith("Lorem ipsum") and t["font"]["weight"] >= 700)
    )


for slug, name in names.items():
    d = json.loads((root / "design-source/reference-data" / f"{slug}.json").read_text())
    texts = [t for t in d["texts"] if t["scope"] == "page"]
    fs = []
    if slug in labels:
        page_labels = labels[slug].split("|")
        assert len(page_labels) == len(texts), (slug, len(page_labels), len(texts))
    else:
        titles = [t for t in texts if t["text"].startswith("Lorem ipsum")]
        page_labels = []
        for t in texts:
            if t in titles:
                label = f"Card {titles.index(t)+1} — Title"
            elif t["text"].startswith("Excepteur"):
                title = min(
                    titles, key=lambda a: abs(a["x"] - t["x"]) + abs(a["y"] - t["y"])
                )
                label = f"Card {titles.index(title)+1} — Excerpt"
            elif t["tag"] == "h1":
                label = "Introduction — Heading"
            elif slug == "blog" and 600 < t["y"] < 750:
                label = f'Filters — {t["text"].strip()} Label'
            elif t["text"] == "View More":
                label = "Pagination — Button Label"
            elif t["text"].startswith("Schedule"):
                label = "Introduction — Button Label"
            elif t["y"] < 350:
                label = "Introduction — Description"
            else:
                label = (
                    "Introduction — Consultation Text"
                    if slug == "case-studies"
                    else "Introduction — Description"
                )
    ...
    for t, label in zip(texts, page_labels):
        fs.append(
            field(
                t["key"],
                label,
                rows=max(2, min(8, len(t["lines"]))),
                new_lines="",
                instructions="Edit the page text here. Keep deliberate line breaks for the designed layout.",
            )
        )
        if (
            slug == "blog"
            and t["text"].startswith("Lorem ipsum")
            and t["font"]["weight"] >= 700
        ):
            fs.append(
                field(
                    t["key"] + "_category",
                    label.replace("Title", "Category"),
                    "select",
                    choices={
                        v: v
                        for v in [
        ...
        if link_field(t, slug):
            fs.append(
                field(
                    t["key"] + "_url",
                    (
                        label.replace("Button Label", "Button URL") + " — Link"
                        if "Button Label" not in label
                        else label.replace("Button Label", "Button URL")
                    ),
                    "text",
                    instructions="Full URL, relative site URL, or tel: link. Leave blank to use the default destination.",
                )
            )
        if (
            ...
            fs.append(
                field(t["key"] + "_answer", label.replace("Question", "Answer"), rows=4)
            )
    for i, im in enumerate(d["images"]):
        label = (
            image_labels[slug][i]
            if slug in image_labels
            else (
                "Hero — Image"
                if slug == "blog" and i == 0
                else f'Card {i if slug=="blog" else i+1} — Image'
            )
        )
        fs.append(
            field(
                ...
                preview_size="medium",
                library="all",
                instructions="Choose a replacement image. The original design asset is used when empty.",
            )
        )
    groups.append(
        dict(
            key="group_ps_" + slug,
            title=name + " — Page Content",
            fields=fs,
            location=[
                [
                    dict(
            ...
            style="default",
            active=True,
            description="Editable page fields. Keep field names and keys unchanged to preserve template connections.",
        )
    )
    for t in d["texts"]:
        if t["key"] in shared_labels:
            shared[t["key"]] = field(
                t["key"], shared_labels[t["key"]], rows=2, new_lines=""
            )
shared["booking"] = field(
    "ps_booking_url",
    "Default Page Button and Footer Call URL",
    "text",
    default_value="tel:+14044259775",
)
groups.append(
    dict(
        key="group_ps_shared",
        title="Site Content — Footer and Default Links",
        fields=list(shared.values()),
        location=[[dict(param="post_type", operator="==", value="ps_site_content")]],
        active=True,
        description="Navigation menus are managed under Appearance → Menus.",
    )
)
(theme / "acf-import/field-groups.json").write_text(
    json.dumps(groups, indent=2, ensure_ascii=False) + "\n"
)
print(
    "Created",
    len(groups),
    "editable ACF groups with",
