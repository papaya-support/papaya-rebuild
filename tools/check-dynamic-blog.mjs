...
await instance.playground.run({code:`<?php require '/wordpress/wp-load.php';stick_post(${fixture.ids[0]});`});
await page.goto(url,{waitUntil:'domcontentloaded'});
if(await page.locator('.post-card').first().getAttribute('data-post-id')!==String(fixture.ids[0]))throw Error('Older sticky post not first');
await page.goto(url+'?blog_page=2',{waitUntil:'domcontentloaded'});
if(await page.locator(`[data-post-id="${fixture.ids[0]}"]`).count()||await page.locator('.post-card').count()!==3)throw Error('Sticky post duplicated or pagination changed');
await page.goto(url+'?blog_category=qa-paid',{waitUntil:'domcontentloaded'});
if(await page.locator('.post-card').count()!==1||await page.locator('.post-card').first().getAttribute('data-post-id')!==String(fixture.ids[11]))throw Error('Sticky post leaked into unrelated category');
