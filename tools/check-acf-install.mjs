...
echo json_encode(['groups'=>count($groups),'verifiedFieldDefinitions'=>$checked,'existingContentPreserved'=>true,'databaseGroups'=>count(get_posts(['post_type'=>'acf-field-group','posts_per_page'=>-1]))]);`});console.log(result.text);if(result.errors||result.exitCode)throw Error(result.errors||result.text);fs.writeFileSync('verification/acf-install-checks.json',result.text);}finally{await instance[Symbol.asyncDispose]();}
