...
const results=[];
page.on('response', response => {if(response.request().isNavigationRequest() && response.status() >= 400) errors.push(`HTTP ${response.status()}: ${response.url()}`);});
for(const width of [1280,390]){
 ...
 for(const d of pages){
  ...
  const state=await page.evaluate(expected=>({h1:document.querySelectorAll('main h1').length,footer:!!document.querySelector('footer'),fatal:/Fatal error|There has been a critical error/.test(document.body.textContent),svg:document.querySelectorAll('main svg,footer svg').length,inline:document.querySelectorAll('[style]').length,overflow:document.documentElement.scrollWidth>innerWidth,missingFields:expected.filter(k=>!document.querySelector(`[data-field="${k}"],[data-image="${k}"]`)),missingImages:[...document.images].filter(i=>i.complete&&!i.naturalWidth).map(i=>i.src),height:document.body.scrollHeight}),expected);
  if(!state.footer||state.fatal||state.h1!==1||state.svg||state.inline||state.overflow||state.missingFields.length||state.missingImages.length)throw Error(JSON.stringify({slug:d.slug,width,...state}));
...
fs.writeFileSync('verification/native-checks.json',JSON.stringify({pages:results,errors,faq:true,mobileNavigation:true},null,2));console.log('Verified eight semantic HTML pages at desktop and mobile sizes; all ACF fields and images present, no SVG layouts, inline styles, overflow, or JS errors.');await browser.close();
