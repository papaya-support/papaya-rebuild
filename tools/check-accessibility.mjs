...
echo json_encode([get_permalink($id),get_permalink($plain),get_category_link(1)]);`,
  ...
  for (const width of [1280, 390]) {
    ...
    for (const url of urls) {
      await page.goto(new URL(url, 'http://127.0.0.1:9479').href);
      await page.addScriptTag({ path: path.resolve('tools/node_modules/axe-core/axe.min.js') });
      const data = await page.evaluate(async () => {
        ...
        return {
          ...
          images: [...document.querySelectorAll('main img')].map((i) => ({
            ...
            responsive: !!i.srcset,
            loading: i.loading,
            broken: i.complete && !i.naturalWidth,
          })),
  ...
  if (await page.locator('main').evaluate((e) => e !== document.activeElement))
    throw Error('Skip link did not focus main');
  ...
  if (
    ...
    throw Error('Mobile menu focus restoration');
  await page.screenshot({ path: '/tmp/papaya-accessibility-mobile.png', fullPage: true });
  await page.setViewportSize({ width: 1280, height: 900 });
  await page.goto('http://127.0.0.1:9479/blog-detail/');
  await page.screenshot({ path: '/tmp/papaya-accessibility-desktop.png', fullPage: true });
  const noJS = await browser.newPage({
    javaScriptEnabled: false,
    viewport: { width: 390, height: 900 },
  });
  await noJS.goto('http://127.0.0.1:9479/');
  if (!(await noJS.locator('#primary-navigation').isVisible()))
    throw Error('No-JavaScript mobile navigation inaccessible');
  await noJS.close();
  fs.mkdirSync('verification', { recursive: true });
  fs.writeFileSync('verification/accessibility-audit.json', JSON.stringify(report, null, 2));
  console.log(
    JSON.stringify(
      report.map((r) => ({
        url: r.url,
        width: r.width,
        h1: r.headings.filter((h) => h.level === 1).length,
        gaps: r.headings.filter((h, i) => i && h.level > r.headings[i - 1].level + 1),
        overflow: r.overflow,
  ...
  const failures = report.filter(
    (r) =>
      ...
      !r.title ||
      !r.lang ||
      r.headings.filter((h) => h.level === 1).length !== 1 ||
      r.headings.some((h, i) => !h.text || (i && h.level > r.headings[i - 1].level + 1)) ||
      r.violations.some((v) => v.id !== 'color-contrast'),
  );
  console.log(
    `${report.length} viewport checks; ${failures.length} structural/non-contrast failures. Contrast exceptions retained per user instruction.`,
  );
