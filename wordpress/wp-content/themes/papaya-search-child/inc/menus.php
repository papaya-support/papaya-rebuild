...
{
    return $t['scope'] === 'header' ||
        in_array(
            $t['key'],
            ['shared_819c8f0b4b41', 'shared_5f04f2b25bf0', 'shared_ed5745efb63b'],
            true,
        );
}
...
{
    if (get_option('ps_menus_migrated') || !get_option('ps_design_imported')) {
        return;
    }
    $design = ps_initial_content('home');
    $sets = array_fill_keys(array_keys(ps_menu_locations()), []);
    foreach ($design['texts'] as $t) {
        ...
        if ($t['scope'] === 'header') {
            $url =
                ps_value($t['key'] . '_url', '', $t['scope']) ?:
                (preg_match('/Get Started/i', $t['text'])
                    ? ps_booking_url()
                    : ps_link_for_label($t['text']));
            $sets['primary'][] = [
                'title' => $value,
                'url' => $url,
                'classes' => preg_match('/Get Started/i', $t['text']) ? 'menu-cta' : '',
            ];
        }
        $location =
            [
                'shared_819c8f0b4b41' => 'footer_navigation',
                'shared_5f04f2b25bf0' => 'footer_services',
            ][$t['key']] ?? '';
        if ($location) {
            foreach (explode("\n", $value) as $label) {
                if (trim($label) !== '') {
                    $sets[$location][] = [
                        'title' => trim($label),
                        'url' => ps_link_for_label(trim($label)) ?: '#',
                    ];
                }
            }
        }
        if ($t['key'] === 'shared_ed5745efb63b') {
            foreach (preg_split('/\n\s*\n/', trim($value)) as $label) {
                $label = trim($label);
                $url = ps_link_for_label($label);
                if (!$url && preg_match('/^[+\d() .-]+$/', $label)) {
                    $url = 'tel:' . preg_replace('/[^+\d]/', '', $label);
                }
                if (!$url) {
                    $url =
    ...
    foreach (
        [
            'Facebook' => 'https://www.facebook.com/papayasearch',
            'Twitter' => 'https://twitter.com/PapayaSearch',
            'LinkedIn' => 'https://www.linkedin.com/company/papayasearch/',
        ]
        as $label => $url
    ) {
        $sets['footer_social'][] = [
            'title' => $label,
            'url' => $url,
            'classes' => 'social-' . strtolower($label),
        ];
    }
...
function ps_render_menu($location, $mobile = false)
{
    ...
    echo '<nav class="ps-menu ps-menu-' .
        esc_attr($location) .
        '" aria-label="' .
    ...
    wp_nav_menu([
        ...
        'menu_id' => ($mobile ? 'mobile-' : 'desktop-') . $location,
        'fallback_cb' => false,
        'depth' => 0,
    ]);
    echo '</nav>';
...
add_filter(
    ...
    function ($atts, $item, $args) {
        if (($args->theme_location ?? '') === 'footer_social') {
