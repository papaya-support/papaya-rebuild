...
function ps_article_headings($html)
{
    ...
    return preg_replace_callback(
        '~<!--[\s\S]*?-->|<(pre|code|script|style)\b[^>]*>[\s\S]*?</\1\s*>|<(\/?)(h[1-6])\b([^>]*)>~i',
        function ($match) use (&$stack, &$levels) {
            if (empty($match[3])) {
                return $match[0];
            }
            $original = (int) substr($match[3], 1);
            if ($match[2] === '/') {
                return '</h' . ($levels[$original] ?? max(2, $original)) . '>';
            }
            while ($stack && end($stack)['original'] >= $original) {
                array_pop($stack);
            }
            $level = min(6, $stack ? end($stack)['level'] + 1 : 2);
            $stack[] = ['original' => $original, 'level' => $level];
            $levels[$original] = $level;
            $tag = '<h' . $level . $match[4] . '>';
            if ($level !== $original) {
                $processor = new WP_HTML_Tag_Processor($tag);
                $processor->next_tag();
                $processor->add_class('editor-heading-' . $original);
