...
function ps_pages()
{
    return array_values(
        array_map(
            fn($page) => ['name' => $page['name'], 'slug' => $page['slug']],
            ps_initial_content(),
        ),
    );
}
function ps_shared_id()
{
    return (int) get_option('ps_shared_content_id');
}
function ps_value($key, $default = '', $scope = 'page', $post_id = null)
{
    ...
    if ($id && metadata_exists('post', $id, $key)) {
        return function_exists('get_field')
            ? get_field($key, $id, false)
            : get_post_meta($id, $key, true);
    }
    return $default;
}
function ps_asset($asset)
{
    return get_stylesheet_directory_uri() . '/assets/images/' . $asset;
}
...
function ps_destinations()
{
    return [
        ...
        'About' => ps_route('about'),
        'Blog' => ps_route('blog'),
        'Free Tools' => 'https://papayasearch.com/tools/',
        'Careers' => 'https://papayasearch.com/careers/',
        'AI SEO' => 'https://papayasearch.com/services/ai-seo-aeo-services/',
        'SEO' => 'https://papayasearch.com/services/seo/',
        'SEO for SaaS' => 'https://papayasearch.com/industry/saas-seo-agency/',
        'Atlanta SEO' => 'https://papayasearch.com/atlanta-seo-agency/',
        'Atlanta SEM' => 'https://papayasearch.com/atlanta-sem-agency/',
        'PPC' => ps_route('search-engine-marketing'),
