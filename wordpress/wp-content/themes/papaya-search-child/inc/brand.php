...
add_filter('astra_get_option_global-color-palette', function ($value) {
    ...
    $value['palette'] = array_map(fn($slug) => $colors['papaya-' . $slug], [
        'orange',
        'light-orange',
        'teal',
        'teal',
...
add_action(
    'after_setup_theme',
    function () {
        add_theme_support('editor-color-palette', ps_brand_palette());
    },
    30,
);
add_filter('tiny_mce_before_init', function ($settings) {
    $map = [];
    foreach (ps_brand_palette() as $color) {
...
add_action('admin_init', function () {
    if (!current_user_can('manage_options') || get_option('ps_brand_emblem_v1')) {
        return;
    }
    ...
    if (
        ...
        'f86f7af73c0fd32b172d6d0ee811a7f5730cfd30e2ae259fa3744a2d9ecf8210'
    ) {
        update_option('ps_brand_emblem_v1', 1, false);
        return;
    }
    $upload = wp_upload_bits(
        'papaya-brand-emblem.png',
        null,
        file_get_contents(get_stylesheet_directory() . '/assets/images/hero-emblem.png'),
    );
