...
?></div>
    ...
    <?php
        endwhile;
         ...
         ?>
    <h1><?php if (is_404()) {
    esc_html_e('Page not found', 'papaya-search');
} elseif (is_search()) {
    echo esc_html(sprintf(__('Search results for: %s', 'papaya-search'), get_search_query()));
} elseif (is_archive()) {
    echo esc_html(wp_strip_all_tags(get_the_archive_title()));
} else {
    esc_html_e('Blog', 'papaya-search');
} ?></h1>
    <?php if (have_posts()): ?>
    ...
    'papaya-search',
); ?> <a href="<?php echo esc_url(home_url('/')); ?>">Return to the home page</a></p>
    <?php endif;
    endif; ?>
</main>
<?php get_footer(); ?>
